...
categories: [kvm,not_present_pf_bypass]
tags:       [not_present_pf_bypass]
---
...
> kvm 捕获两类PF：
 - host page faults, where the fault is needed to allow kvm to install
   ...
   > host PF，需要该fault 来允许 kvm install shadow  pte 或更新来guest accessed
   > 和 dirty bit
 - guest page faults, where the guest has faulted and kvm simply injects
   the fault back into the guest to handle
   > guest PF, guest 出现fault ，kvm 只是将fault 注入guest 来处理

The second class, guest page faults, is pure overhead.  We can eliminate
some of it on vmx using the following evil trick:
> pure [pjʊr]: 纯的; 纯粹的; 纯净的; 完全的;
> trick: 技巧; 把戏; 戏法; 花招; 诡计; 骗局; 一圈;
> eliminate [ɪˈlɪmɪneɪt]: 消除; 排除; 消灭，干掉(尤指敌人或对手); 清除; (比赛中)淘汰
>
> 第二类，访客页面错误，是纯粹的开销。 我们可以使用以下邪恶技巧在 vmx  上消除其中
> 的一些：
...
> discriminate [dɪˈskrɪmɪneɪt]: 区分
>
> 不幸的是，这个技巧仅适用于 Intel 硬件，因为 AMD 缺乏一种根据error code 区
> 分PF的方法。它也有一点风险，因为它使用了将来可能变为非保留的保留位，因此提供了
> 一个模块参数来禁用它。
...
@@ -2582,6 +2586,9 @@ static int __init vmx_init(void)
 	if (r)
 		goto out1;
	//0xffe,     1111 1111 1110
	//取非: 11...0000 0000 0001
	//这样, 如果 PFEC & PFEC_MASK = PFEC_MATCH, 则可能会产生VM-exit(需要看exception
	//bitmap的bit 14 是否为1), 而 如果spte 设置了~0xffeull, 则PFEC判断逻辑时,先看present, 这里
	//为1, 再看reserved bit, 发现set了, 所以会将PFEC P(bit 0)位设置为1, 然后再设置RSVD 位(bit3).
	//此时上面的条件满足(1 & 1 = 1),  产生 VM-exit, 这种用在guest的pte 本身是present的
	//
	//还有一种情况是,  guest pte是present的, 这时将spte设置为0, 则 PFEC为 0, 此时上述条件不满足, 
	//(0 & 1 != 1), 不会产生VM-exit, 达到了bypass 的目的.
