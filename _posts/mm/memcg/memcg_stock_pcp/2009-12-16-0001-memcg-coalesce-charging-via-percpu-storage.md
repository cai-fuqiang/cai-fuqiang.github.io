---
layout:     post
title:      "[PATCH] memcg: coalesce charging via percpu storage"
author:     "fuqiang"
date:       "Tue, 15 Dec 2009 16:47:08 -0800"
categories: [memcg]
tags:       [memcg_stock_pcp]
---
...
Date: Tue, 15 Dec 2009 16:47:08 -0800
Subject: [PATCH] memcg: coalesce charging via percpu storage
...
这是一项用于通过每 CPU 缓存合并对 res_counter 的访问的补丁。在charge 时, 
memcg 一次性charge 64 页，并将其存储在每 CPU 缓存中。由于这是缓存，因此
在必要时需要进行清空或刷新。
...
This version uses public percpu area.
 ...
 1. Sum of stocked charge in the system is limited to # of cpus
    ...
    > 系统中累计的charge总量受到 CPU 数量的限制，而不是 memcg 的数量。这表明
    > 同步效果更好。
 2. drain code for flush/cpuhotplug is very easy (and quick)
    > 用于刷新/CPU 热插拔的清空代码非常简单（且执行迅速）。
...
> 这个补丁最重要的一点是，我们在快速路径中从不触碰 res_counter。res_counter 
> 是一个系统范围内共享的计数器，经常被修改。为了避免伪共享，我们应该尽可能避
> 免对它的访问。
...
> 你在 x86-64 8 CPU 服务器上测试 memcg 在页面故障时的开销，通过运行一个在循
> 环中执行映射（map）、故障（fault）和取消映射（unmap）的程序来进行测试。你使用
> taskset 在每个 CPU 上运行一个任务，并测量 60 秒内页面故障的总数。
...
@@ -1258,6 +1262,133 @@ void mem_cgroup_update_mapped_file_stat(struct page *page, int val)
...
@@ -2469,6 +2611,7 @@ static int mem_cgroup_force_empty(struct mem_cgroup *mem, bool free_all)
 			goto out;
